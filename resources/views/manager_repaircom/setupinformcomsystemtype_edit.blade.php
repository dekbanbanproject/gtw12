...
?>
        ...
        <form  method="post" action="{{ route('mrepaircom.setting_typesystem_update') }}" enctype="multipart/form-data">
        @csrf
        <div class="row push">
    ...
    <div class="col-lg-1">
      ...
      <div class="col-lg-3">
      <input  name = "INFORMCOM_ST_NAME"  id="INFORMCOM_ST_NAME" class="form-control input-lg" style=" font-family: 'Kanit', sans-serif;" value="{{$infosystem->INFORMCOM_ST_NAME}}" >
      <div style="color: red; font-size: 16px;" id="article_name"></div> 
    </div>
    
      <input  type="hidden" name = "INFORMCOM_ST_ID"  id="INFORMCOM_ST_ID" class="form-control input-lg" value="{{$infosystem->INFORMCOM_ST_ID}}">
      </div></div>
        <div class="modal-footer">
        <div align="right">
        <button type="submit"  class="btn btn-hero-sm btn-hero-info" >บันทึกแก้ไขข้อมูล</button>
         <a href="{{ url('manager_repaircom/repaircominfosetting_typesystem')  }}" class="btn btn-hero-sm btn-hero-danger" onclick="return confirm('ต้องการที่จะยกเลิกการแก้ไขข้อมูล ?')" >ยกเลิก</a> 
         </div>    
